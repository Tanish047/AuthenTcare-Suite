...
const MarketList = ({ 
  ...
  const availableMarkets = allMarkets.filter(market => 
    !versionMarkets.some(vm => vm.id === market.id)
  );

  // Get markets already added to this version
  const addedMarkets = allMarkets.filter(market => 
    versionMarkets.some(vm => vm.id === market.id)
  );
  ...
  const handleBulkDeleteConfirm = async () => {
    ...
    try {
      ...
      setBulkDeleteError('');
      setShowSuccessMessage(`Successfully removed ${marketIds.length} market${marketIds.length > 1 ? 's' : ''} from version`);
      setTimeout(() => setShowSuccessMessage(false), 3000);
    } catch (error) {
    ...
    <div style={{ marginTop: 0 }}>
      ...
      {showSuccessMessage && (
        <div style={{
          ...
          ✓ {typeof showSuccessMessage === 'string' ? showSuccessMessage : 'Market added successfully!'}
        </div>
      )}
            ...
            <li key={market.id} style={{ marginBottom: 12 }}>
              <div
                ...
                style={{
                  padding: '16px',
                  border: '1px solid ' + (
                    isSelectionMode && selectedMarkets.has(market.id) ? '#17a2b8' :
                    selectedMarket?.id === market.id ? '#2c5aa0' : '#e0e0e0'
                  ),
                  borderRadius: '8px',
                  cursor: 'pointer',
                  backgroundColor: 
                    isSelectionMode && selectedMarkets.has(market.id) ? '#e7f3ff' :
                    selectedMarket?.id === market.id ? '#f0f7ff' : '#fff',
                ...
                <div style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ 
                      ...
                      {isSelectionMode ? 'Click to select/deselect' : 'Click to proceed to license selection'}
                    </div>
                  </div>
                </div>
        ...
        >
          ...
          >
            ...
            <p style={{ marginBottom: '16px', color: '#333' }}>
              Are you sure you want to remove <strong>{selectedMarkets.size} market{selectedMarkets.size > 1 ? 's' : ''}</strong> from this version? 
              This will not delete the markets from the database, only remove them from this specific version.
            </p>
