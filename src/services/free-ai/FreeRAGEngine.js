...
class FreeRAGEngine {
  ...
  async initialize() {
    try {
      ...
      await Promise.all([this.aiService.initialize(), this.vectorDB.initialize()]);
  ...
  async query(question, options = {}) {
    try {
      ...
      if (searchResult.success && searchResult.results.length > 0) {
        ...
        context = searchResult.results.map(result => result.content).join('\n\n');
      ...
      return {
        ...
        error: error.message,
        content:
          'I apologize, but I encountered an error processing your query. Please try again or check the system configuration.',
      };
    }
  }
