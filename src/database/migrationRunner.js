  ...
  try {
    ...
    if (marketCount.count === 0) {
      const defaultMarkets = [
        ...
        { name: 'Canada (Health Canada)', region: 'North America', regulatory_body: 'Health Canada' },
      ...
      for (const market of defaultMarkets) {
        await db.run(
          ...
          [market.name, market.region, market.regulatory_body]
        );
      }
    ...
    if (notificationCount.count === 0) {
      await db.run(
        ...
        ['Welcome to AuthenTcare Suite', 'Your regulatory management journey starts here!', 'info']
      );
