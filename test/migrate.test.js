...
test('migrate should be idempotent', async () => {
  ...
  try {
    ...
    const migrationTable = await db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='__migrations__'");
...
test('migrate should create expected tables', async () => {
  ...
  try {
    ...
    const expectedTables = ['projects', 'devices', 'versions', 'markets', 'licenses', 'clients', 'news', 'events', 'notifications'];
    ...
    // Check for FTS table
    const ftsTable = await db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='projects_fts'");
